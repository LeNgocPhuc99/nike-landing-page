...
const Hero = () => {
  ...
  const onChangeBigShoe = (newBigShoe: string) => {
    setBigShoeImg(newBigShoe);
  };
        ...
        <div className="flex justify-start items-start flex-wrap w-full mt-20 gap-16">
          {statistics.map((item) => (
            <div key={item.label}>
              ...
              <p className="text-heading-7 font-montserrat text-slate-gray">
                {item.label}
              </p>
            </div>
          ))}
